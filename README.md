...
> **当前状态说明**：项目存在一点小问题，导致 **action 未能正确执行**（策略下发的动作在仿真中未按预期生效）。数据采集与格式转换流程可正常使用。后续会排查并修复动作执行链路。

### 演示

| 数据采集 | 策略执行（action 尚未正确生效） |
|----------|----------------------------------|
| ![数据采集演示](data_collection.gif) | ![策略执行演示](actiono.gif) |
